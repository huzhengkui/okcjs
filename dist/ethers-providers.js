...
utils.defineProperty(EtherscanProvider.prototype, 'perform', function(method, params) {
    ...
    switch (method) {
        case 'getBlockNumber':
            url += '/api?module=proxy&action=okc_blockNumber' + apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);

        case 'getGasPrice':
            url += '/api?module=proxy&action=okc_gasPrice' + apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);
        ...
        case 'getTransactionCount':
            url += '/api?module=proxy&action=okc_getTransactionCount&address=' + params.address;
            url += '&tag=' + params.blockTag + apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);

        case 'getCode':
            url += '/api?module=proxy&action=okc_getCode&address=' + params.address;
            url += '&tag=' + params.blockTag + apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);

        case 'getStorageAt':
            url += '/api?module=proxy&action=okc_getStorageAt&address=' + params.address;
            url += '&position=' + params.position;
            url += '&tag=' + params.blockTag + apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);

        case 'sendTransaction':
            url += '/api?module=proxy&action=okc_sendRawTransaction&hex=' + params.signedTransaction;
            url += apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);
        ...
        case 'getBlock':
            if (params.blockTag) {
                url += '/api?module=proxy&action=okc_getBlockByNumber&tag=' + params.blockTag;
                url += '&boolean=false';
                url += apiKey;
                return Provider.fetchJSON(url, null, getJsonResult);
            }
        ...
        case 'getTransaction':
            url += '/api?module=proxy&action=okc_getTransactionByHash&txhash=' + params.transactionHash;
            url += apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);

        case 'getTransactionReceipt':
            url += '/api?module=proxy&action=okc_getTransactionReceipt&txhash=' + params.transactionHash;
            url += apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);
        ...
        case 'call':
            ...
            url += '/api?module=proxy&action=okc_call' + transaction;
            url += apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);
        ...
        case 'estimateGas':
            var transaction = getTransactionString(params.transaction);
            if (transaction) { transaction = '&' + transaction; }
            url += '/api?module=proxy&action=okc_estimateGas&' + transaction;
            url += apiKey;
            return Provider.fetchJSON(url, null, getJsonResult);
...
utils.defineProperty(JsonRpcSigner.prototype, 'getAddress', function() {
    ...
    return this.provider.send('okc_accounts', []).then(function(accounts) {
...
utils.defineProperty(JsonRpcSigner.prototype, 'sendTransaction', function(transaction) {
    ...
    return this.getAddress().then(function(address) {
        ...
        return provider.send('okc_sendTransaction', [ transaction ]).then(function(hash) {
            return new Promise(function(resolve, reject) {
                function check() {
...
utils.defineProperty(JsonRpcSigner.prototype, 'signMessage', function(message) {
    ...
    return this.getAddress().then(function(address) {
        ...
        // https://github.com/ethereum/wiki/wiki/JSON-RPC#okc_sign
        return provider.send('okc_sign', [ address.toLowerCase(), utils.hexlify(data) ]);
    });
});
...
utils.defineProperty(JsonRpcProvider.prototype, 'listAccounts', function() {
    return this.send('okc_accounts', []).then(function(accounts) {
...
utils.defineProperty(JsonRpcProvider.prototype, 'perform', function(method, params) {
    switch (method) {
        case 'getBlockNumber':
            return this.send('okc_blockNumber', []);

        case 'getGasPrice':
            return this.send('okc_gasPrice', []);

        case 'getBalance':
            return this.send('okc_getBalance', [getLowerCase(params.address), params.blockTag]);

        case 'getTransactionCount':
            return this.send('okc_getTransactionCount', [getLowerCase(params.address), params.blockTag]);

        case 'getCode':
            return this.send('okc_getCode', [getLowerCase(params.address), params.blockTag]);

        case 'getStorageAt':
            return this.send('okc_getStorageAt', [getLowerCase(params.address), params.position, params.blockTag]);

        case 'sendTransaction':
            return this.send('okc_sendRawTransaction', [params.signedTransaction]);

        case 'getBlock':
            if (params.blockTag) {
                return this.send('okc_getBlockByNumber', [params.blockTag, false]);
            } else if (params.blockHash) {
                return this.send('okc_getBlockByHash', [params.blockHash, false]);
            }
            return Promise.reject(new Error('invalid block tag or block hash'));

        case 'getTransaction':
            return this.send('okc_getTransactionByHash', [params.transactionHash]);

        case 'getTransactionReceipt':
            return this.send('okc_getTransactionReceipt', [params.transactionHash]);

        case 'call':
            return this.send('okc_call', [getTransaction(params.transaction), 'latest']);

        case 'estimateGas':
            return this.send('okc_estimateGas', [getTransaction(params.transaction)]);

        case 'getLogs':
            if (params.filter && params.filter.address != null) {
                params.filter.address = getLowerCase(params.filter.address);
            }
            return this.send('okc_getLogs', [params.filter]);
...
utils.defineProperty(JsonRpcProvider.prototype, '_startPending', function() {
    ...
    var pendingFilter = this.send('okc_newPendingTransactionFilter', []);
    this._pendingFilter = pendingFilter;

    pendingFilter.then(function(filterId) {
        function poll() {
            self.send('okc_getFilterChanges', [ filterId ]).then(function(hashes) {
                ...
                if (self._pendingFilter != pendingFilter) {
                    self.send('okc_uninstallFilter', [ filterIf ]);
                    return;
                }
...
utils.defineProperty(Web3Provider.prototype, 'send', function(method, params) {
    ...
    // Metamask complains about okc_sign (and on some versions hangs)
    if (method == 'okc_sign' && this._web3Provider.isMetaMask) {
